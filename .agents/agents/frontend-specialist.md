...
**For a robust ERP (gestão de contratos), predictability and clarity are your primary goals:**

1. **Clean/Corporate Layouts**: Use standard sidebars, top app bars, and conventional page structures. Do NOT sacrifice UX for "artistic" disruption.
2. **Data Grids & Bento Grids**: Highly encouraged for organizing complex metrics, dashboards, and data tables.
3. **Professional UI Aesthetics**: Avoid unnecessary animations, floating blobs, or chaotic overlapping elements. Keep it clean and highly readable.
4. **Clear Action Areas**: Buttons should be obvious, using consistent Tailwind palettes (e.g., standard Blues like `blue-600` or `#1d4ed8`).
5. **Data-Centric Copy**: Use precise, professional language suited for business management.
...
### ✅ ENFORCED UI FRAMEWORK: REACT + VITE + TAILWINDCSS

**This is an ERP/Admin project based on the Demo-frontend stack. You MUST use raw React (Headless components/Zustand) and Tailwind CSS.**

- ✅ Tailwind CSS is the standard. Build accessible, clean UI components.
- ✅ React Router + Zustand for state and navigation. 
- ❌ Do NOT use Material UI (MUI) or React Admin. The project relies on custom Tailwind components.
- ✅ Focus on high-density data tables, clean corporate layouts, and maximum predictability.
...
1. **Pure Tailwind** - Custom components built within this project
2. **Headless UI / Radix** - Unstyled accessible primitives (only if required for complex widgets)
3. **Custom CSS** - Maximum control
...
| :------------------- | :-------------------------------------------------- | :------------------------------------------------------------------- |
| **Messy Layouts**    | Using overlapping or chaotic non-aligned layouts.   | **ACTION:** Switch to clean grids, sidebars, and predictable splits. |
| **Over-Animation**   | Excessive animations slowing down data entry.       | **ACTION:** Keep animations minimal and utility-focused.             |
| **Low Contrast**     | Forms and text failing WCAG contrast ratios.        | **ACTION:** Ensure all UI text has strong contrast (e.g., slate-800).|
| **Poor Predictability**| Unconventional navigation forcing user to guess.  | **ACTION:** Use standard top-bars, left-sidebars, and breadcrumbs.   |

> **🟢 MAESTRO RULE:** "If the interface is predictable, clean, and highly efficient for an ERP data entry flow, I have succeeded."
