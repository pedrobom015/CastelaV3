...
import { AuxilioFuneral } from "./pages/plano/lancamentos/AuxilioFuneral";
...
import { SeedClasses } from "./pages/plano/apoio/SeedClasses";
import { SeedGrupo } from "./pages/plano/apoio/SeedGrupo";
import { SeedProcessos } from "./pages/plano/apoio/SeedProcesso";
...
export default function App() {
	...
	useEffect(() => {
		const waitHydration = async () => {
			...
			if (IS_DEMO) {
				const cached = useAppStore.getState().tables;
				const hasCached = cached instanceof Map && cached.size > 0;

				if (hasCached) {
					// Cache-first: mostra o app imediatamente com os dados do IDB
					setDirHandle(DEMO_DIR_HANDLE, "backend");
					setIsReady(true);
					// Refresh silencioso em background — atualiza sem loading
					loadTables()
						.then((loaded) => setTables(loaded))
						.catch((e) =>
							console.warn(
								"[demo] Background refresh falhou:",
								e,
							),
						);
				} else {
					// Primeira vez: carrega do backend normalmente
					try {
						...
						console.error(
							"[demo] Falha ao carregar dados do backend:",
							e,
						);
					}
					setDirHandle(DEMO_DIR_HANDLE, "backend");
		...
		return (
			<div className="min-h-screen bg-gray-800 flex items-center justify-center">
				<div className="text-white flex flex-col items-center gap-2">
					...
					<p>
						Carregando
						{IS_DEMO ? " dados do servidor" : " banco de dados"}...
					</p>
				</div>
			</div>
		);
					...
					<Route element={<AppShell />}>
						...
						<Route element={<MainLayout />}>
							<Route
								path="/dashboard"
								element={
									<EmDesenvolvimento title="Dashboard" />
								}
							/>
							...
							<Route
								...
								element={
									<Em title="Cancelamentos por Vendedor" />
								}
							/>
							...
							<Route
								...
								element={
									<Em title="Eliminar Custos Adicionais" />
								}
							/>
							<Route
								path="/cobranca/impressao/cobrador"
								element={
									<Em title="Impressão Modelo por Rateio" />
								}
							/>
							<Route
								path="/cobranca/impressao/periodico"
								element={
									<Em title="Impressão Modelo Periódico" />
								}
							/>
							<Route
							...
							<Route
								...
								element={
									<Em title="Eliminar Registros Apagados" />
								}
							/>
							<Route
								path="/apoio/senhas"
