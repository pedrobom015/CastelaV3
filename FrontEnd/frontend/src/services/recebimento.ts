...
export function listarTaxasPendentes(
	...
	const jurosMap = new Map<
		string,
		{ multa: number; mltcaren: number; juros: number; jrscaren: number }
	>();
	...
	taxasTable.records.forEach((r, idx) => {
		...
		const emissao = r.emissao_
			? r.emissao_ instanceof Date
				? r.emissao_
				: new Date(r.emissao_ as string)
			: null;
		...
		if (emissao && emissao < hoje) {
			...
			if (jp) {
				if (diasAtraso > jp.mltcaren) {
					multaVal = parseFloat(
						((valor * jp.multa) / 100).toFixed(2),
					);
				}
				if (diasAtraso > jp.jrscaren) {
					jurosVal = parseFloat(
						((valor * diasAtraso * jp.juros) / 100 / 30).toFixed(2),
					);
				}
			}
		}
...
function proximoNumeroRecibo(
	bxrecTable: DbfTable | undefined,
	hoje: Date,
): { ano: string; numero: string } {
...
export async function receberTaxa(
	...
	if (!taxasTable || !gruposTable)
		throw new Error("Tabelas TAXAS e GRUPOS são necessárias");
	...
	const newBxrecTable = bxrecTable
		? {
				...
				header: {
					...bxrecTable.header,
					recordCount: bxrecTable.records.length + 1,
				},
			}
		: {
				header: {
