...
const CustomBulkActionButtons = ({ selectedIds, canDelete }) => {
	if (!canDelete) return null;

	return (
		<BulkDeleteButton
			mutationMode="pessimistic" // evita chamadas duplicadas
		/>
	);
};
...
export const GendersCreate = ({ onClose }) => {
	...
	const handleSave = (values) => {
		create(
			...
			{
				onSuccess: () => {
					notify("Genero criado com sucesso 🎉");
					refresh();
					onClose();
				},
				onError: () =>
					notify("Erro ao criar genero", { type: "error" }),
			}
		);
	};
...
export const GendersEdit = ({ record, onClose }) => {
	...
	const handleSave = (values) => {
		update(
			...
			{
				onSuccess: () => {
					notify("Genero atualizado com sucesso ✅");
					refresh();
					onClose();
				},
				onError: () =>
					notify("Erro ao atualizar genero", { type: "error" }),
			}
		);
	};
	return (
...
export const GendersList = ({ resourceData = "genders" }) => {
			...
			>
				...
				>
					<Datagrid
						...
						rowClick={false}
						bulkActionButtons={
							<CustomBulkActionButtons canDelete={canDelete} />
						}
					>
				...
				>
					...
					<DialogContent>
						<GendersCreate onClose={() => setOpenCreate(false)} />
					</DialogContent>
				</Dialog>
