...
async def get_current_user(
    ...
    """Dependency para obter usu\xa0rio atual autenticado"""
    ...
    if not user:
        raise HTTPException(
            ...
            detail="Usu\xa0rio n?o encontrado"
        )
...
async def get_current_superuser(
    ...
    """Dependency para verificar se usu\xa0rio \x82 superuser"""
    if not current_user.is_superuser:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Acesso restrito a superusu\xa0rios"
        )
    return current_user
