...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get(self, db: Session, id: Any) -> Optional[ModelType]:
        # Para soft delete default, evitaríamos buscar os apagados:
        query = db.query(self.model).filter(getattr(self.model, self.model.__mapper__.primary_key[0].name) == id)
    ...
    def remove(self, db: Session, *, id: int, user_id: Optional[int] = None) -> ModelType:
        # Implementando Soft Delete já que todas as tabelas têm isso
