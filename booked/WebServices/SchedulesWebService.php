...
class SchedulesWebService
{
	...
	public function GetSlots($scheduleId)
	{
		...
		/*SANTOSM START*
		$isUserValid = $scheduleWebServiceView->IsUserValid();
		$isUserSanctioned = $scheduleWebServiceView->IsUserSanctioned();
		/*SANTOSM END*/
		$dates = $scheduleWebServiceView->GetDates();
		$resources = $scheduleWebServiceView->GetResources();

		if ($isError) /*SANTOSM START*|| !$isUserValid || $isUserSanctioned) /*SANTOSM END */
		{
